...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const preload = async () => {
      ...
      console.log(`[App] Pre-loading next trip for ${city}...`);
      
      try {
        // Tenta obter coordenadas se ainda não tiver
        const coords = currentCoords || await getCityCoords(city);
        if (!currentCoords && isMounted && coords && !isNaN(coords[0]) && !isNaN(coords[1])) {
          setCurrentCoords(coords);
        }

        // Chama sem forcedIndex para avançar o ciclo global UMA VEZ
        const generated = await generateTripRequest(city, userData.tripType, undefined, coords);
        
        if (isMounted && city === userData.city) {
          setNextTrip(generated as unknown as TripRequest);
          console.log(`[App] Next trip pre-loaded (Index: ${(generated as any).cycleIndex}/5)`);
        }
      } catch (e) {
        console.error("[App] Erro no pre-load agressivo:", e);
      } finally {
  ...
  }, [nextTrip === null, newRequest === null, acceptedTrip === null, userData.city, userData.tripType]);
  ...
  useEffect(() => {
    if (isOnline && !nextTrip && !newRequest && !acceptedTrip && userData.city) {
      ...
      console.log("[App] Usuário ficou online sem viagem pré-carregada. Iniciando busca...");
      const coords = currentCoords || undefined;
      isFetchingRef.current = true;
      
      generateTripRequest(userData.city, userData.tripType, undefined, coords)
        .then(generated => {
    ...
    const timer = setInterval(async () => {
      ...
      if (dispatchTrigger && !newRequest && !acceptedTrip) {
        ...
        // Se passaram 10 segundos e ainda não temos a viagem, começa a buscar agora
          ...
          console.log("[App] Aviso: Viagem não pré-carregada aos 10s, iniciando busca emergencial...");
          isFetchingRef.current = true;
          generateTripRequest(userData.city, userData.tripType, undefined, currentCoords || undefined)
            .then(generated => {
              if (!nextTripRef.current) setNextTrip(generated as unknown as TripRequest);
            })
            .finally(() => {
