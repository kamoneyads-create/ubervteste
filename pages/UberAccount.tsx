...
const UberAccount: React.FC<UberAccountProps> = ({ 
  ...
  return (
    <div className="h-full bg-white dark:bg-black flex flex-col transition-colors duration-300">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 dark:border-zinc-900">
        ...
        <h1 className="text-[18px] font-bold text-black dark:text-white flex-1 text-center mr-8">
          Conta da Uber
        </h1>
      </div>

      {/* Tabs */}
      <div className="flex overflow-x-auto scrollbar-hide border-b border-gray-100 dark:border-zinc-900 px-2">
        {tabs.map((tab) => (
          <div 
            key={tab.id}
            className={`px-4 py-4 text-[14px] font-medium whitespace-nowrap relative transition-colors ${
              tab.active ? 'text-black dark:text-white' : 'text-gray-500 dark:text-zinc-500'
            }`}
          >
            {tab.label}
            {tab.active && (
              <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-black dark:bg-white" />
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto bg-white dark:bg-black">
        ...
        <div className="flex flex-col items-center pt-10 pb-0">
          <div className="w-[100px] h-[100px] rounded-full overflow-hidden mb-4 border-[3px] border-white dark:border-zinc-800 shadow-sm">
            <img 
              src={profileImage} 
              alt="Profile" 
          ...
          <h2 className="text-[28px] font-bold text-black dark:text-white leading-tight tracking-tight">{userName}</h2>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 px-4 mb-10">
          {gridItems.map((item) => (
            <div 
              key={item.id}
              className="bg-[#F3F3F3] dark:bg-zinc-900 rounded-xl p-4 flex flex-col items-center justify-center text-center aspect-square active:bg-gray-200 dark:active:bg-zinc-800 transition-colors cursor-pointer"
            >
              <div className="mb-3">
                {React.cloneElement(item.icon as React.ReactElement, { className: 'text-black dark:text-white' })}
              </div>
              <span className="text-[14px] font-bold text-black dark:text-white leading-tight">
                {item.label}
              </span>
            </div>
          ))}
