...
const Verification: React.FC<VerificationProps> = ({ 
  ...
  useEffect(() => {
    ...
    if (preloadedVideoUrl && retryCount === 0) {
    ...
    const loadVideo = async () => {
      try {
        ...
        if (videoData && videoData.length > 100) {
          console.log('Vídeo recuperado do banco (Base64)');
  ...
  const handleRetry = async () => {
    console.log('[Verification] Tentando recarregar vídeo do banco...');
    setIsVideoLoading(true);
    ...
    try {
      ...
      if (data && data.length > 100) {
        setVideoUrl(data);
        console.log(`[Verification] Vídeo recarregado com sucesso (${data.length} chars)`);
      } else {
        console.warn('[Verification] Nenhum vídeo válido encontrado no banco durante retry');
        setIsVideoError(true);
      }
    } catch (err) {
      console.error('[Verification] Erro ao recarregar vídeo:', err);
      setIsVideoError(true);
    } finally {
      setIsVideoLoading(false);
    }
  ...
  useEffect(() => {
    if (videoUrl) {
      console.log(`[Verification] videoUrl atualizado: ${videoUrl.substring(0, 50)}... (${videoUrl.length} chars)`);
    } else {
      console.log('[Verification] videoUrl está vazio');
    }
    ...
    if (videoUrl && videoRef.current) {
      ...
      const setupVideo = async () => {
        try {
          ...
          console.log('[Verification] Tentando carregar e reproduzir vídeo...');
          video.load();
          ...
          await video.play();
          console.log('[Verification] Vídeo iniciado com sucesso');
          
          setIsVideoLoading(false);
          setShowManualPlay(false);
        } catch (err) {
          console.warn('[Verification] Auto-play bloqueado ou erro no play:', err);
          setIsVideoLoading(false);
          setShowManualPlay(true);
        }
      };
  ...
  return (
    <div className="h-full w-full bg-white flex flex-col relative">
      ...
      {/* TELA DE INTRODUÇÃO */}
      <div className={`absolute inset-0 bg-white flex flex-col text-black overflow-hidden ${stage === 'intro' ? 'opacity-100 z-10' : 'opacity-0 pointer-events-none'}`}>
          {/* Header Superior */}
          <div className="flex justify-between items-center pt-[35px] px-6 pb-0 bg-white sticky top-0 z-50">
          ...
          <div className="flex-1 flex flex-col items-center px-8 pt-0 overflow-y-auto scrollbar-hide">
            ...
            <div className="relative w-full flex justify-center mb-4 mt-0">
      ...
      <div className={`absolute inset-0 bg-white flex flex-col overflow-hidden ${stage !== 'intro' ? 'opacity-100 z-20' : 'opacity-0 pointer-events-none'}`}>
      <style>{`
      ...
      {/* Botão de Voltar */}
      <div className="absolute top-6 left-4 z-[100]">
        <button onClick={onCancel} className="p-3 active:opacity-40 transition-opacity">
          <i className="fa-solid fa-arrow-left text-[24px] text-black"></i>
        </button>
      </div>

      <div className="flex-1 relative flex flex-col items-center justify-start pt-20">
                ...
                {videoUrl && !isVideoError ? (
                  <>
                    <video 
                      ...
                      onError={() => {
                        console.error('Erro ao carregar vídeo de verificação, tentando recuperar...');
                        handleRetry();
                      }}
                    />
